...
const SYMBOL =
    process.env.SYMBOL || 'DOGE/USDT:USDT';

const LEVERAGE =
    Number(process.env.LEVERAGE || 10);

const ORDER_SIZE_USDT =
    Number(process.env.ORDER_SIZE_USDT || 5);

const TIMEFRAME =
    process.env.TIMEFRAME || '5m';

const HTF_TIMEFRAME =
    process.env.HTF_TIMEFRAME || '15m';

const LOOKBACK_CANDLES =
    Number(process.env.LOOKBACK_CANDLES || 200);

const INTERVAL_MINUTES =
    Number(process.env.INTERVAL_MINUTES || 5);
...
const MAX_FUNDING_RATE =
    Number(process.env.MAX_FUNDING_RATE || 0.10) / 100;
...
const ATR_TP_MULTIPLIER =
    Number(process.env.ATR_TP_MULTIPLIER || 1.8);

// ======================================================
// TRAILING
// ======================================================

const TRAILING_CALLBACK_MIN =
    Number(process.env.TRAILING_CALLBACK_MIN || 0.3);

const TRAILING_CALLBACK_MAX =
    Number(process.env.TRAILING_CALLBACK_MAX || 1.5);
...
const TP1_PERCENT =
    Number(process.env.TP1_PERCENT || 30);

const TP2_PERCENT =
    Number(process.env.TP2_PERCENT || 40);

const TP1_RR =
    Number(process.env.TP1_RR || 1.0);

const TP2_RR =
    Number(process.env.TP2_RR || 2.0);
...
const REQUIRED_CONFIRMATION =
    Number(process.env.REQUIRED_CONFIRMATION || 2);

const SIDEWAYS_EMA_GAP =
    Number(process.env.SIDEWAYS_EMA_GAP || 0.04);
...
if (
    process.env.EXCHANGE_DEMO === 'true'
) {

    exchange.enable_demo_trading(true);

    console.log(
        '🧪 DEMO FUTURES ENABLED'
    );
}
...
async function calculateContracts(
    usdt,
    price
) {
...
function calculateCallbackRate(
    atr,
    price
) {
...
`);
    ...
    await retry(() => exchange.createOrder(
        SYMBOL,
        'STOP_MARKET',
        side,
        undefined,
...
async function createPartialTPs(
    position,
    entryPrice,
    atr
) {
