...
const FIBONACCI_RANGE_ADVISOR_CANDLE_CLOSE_BUFFER_MS = Math.max(
  Config.number('FIBONACCI_RANGE_ADVISOR_CANDLE_CLOSE_BUFFER_SECONDS', 5),
  0
) * 1000;
const FIBONACCI_RANGE_ADVISOR_CLUSTER_TOLERANCE_PCT = Config.number(
  'FIBONACCI_RANGE_ADVISOR_CLUSTER_TOLERANCE_PCT',
  0.15
);
const FIBONACCI_RANGE_ADVISOR_MIN_CLUSTER_SCORE = Config.number(
  'FIBONACCI_RANGE_ADVISOR_MIN_CLUSTER_SCORE',
  1
);
const FIBONACCI_RANGE_ADVISOR_MIN_RANGE_WIDTH_PCT = Config.number(
  'FIBONACCI_RANGE_ADVISOR_MIN_RANGE_WIDTH_PCT',
  6
);
const FIBONACCI_RANGE_ADVISOR_MAX_DISTANCE_PCT = Config.number(
  'FIBONACCI_RANGE_ADVISOR_MAX_DISTANCE_PCT',
  25
);
const FIBONACCI_RANGE_ADVISOR_REBUILD_THRESHOLD_PCT = Config.number(
  'FIBONACCI_RANGE_ADVISOR_REBUILD_THRESHOLD_PCT',
  0.5
);
const FIBONACCI_RANGE_ADVISOR_REBUILD_COOLDOWN_MS = Math.max(
  Config.number('FIBONACCI_RANGE_ADVISOR_REBUILD_COOLDOWN_MINUTES', 15),
  0
) * MINUTE_MS;
const FIBONACCI_RANGE_ADVISOR_APPLY_ON = Config.get(
  'FIBONACCI_RANGE_ADVISOR_APPLY_ON',
  'AUTO_RANGE_ONLY'
).toUpperCase();
const FIBONACCI_RANGE_ADVISOR_ALLOW_TRAILING = Config.boolean(
  'FIBONACCI_RANGE_ADVISOR_ALLOW_TRAILING',
  false
);
const FIBONACCI_RANGE_ADVISOR_STATE_FILE = Config.get(
  'FIBONACCI_RANGE_ADVISOR_STATE_FILE',
  'fibonacci-range-advisor-state.json'
);
const FIBONACCI_RANGE_ADVISOR_STATE_PATH = path.resolve(
  process.cwd(),
  FIBONACCI_RANGE_ADVISOR_STATE_FILE
);
...
const DASHBOARD_ENABLED = Config.boolean('DASHBOARD_ENABLED', true);
const DASHBOARD_HOST = Config.get('DASHBOARD_HOST', '127.0.0.1');
const DASHBOARD_PORT = Config.number('DASHBOARD_PORT', 3000);
const DASHBOARD_REFRESH_SECONDS = Math.max(Config.number('DASHBOARD_REFRESH_SECONDS', 5), 2);
const DASHBOARD_CHART_TIMEFRAME = Config.get('DASHBOARD_CHART_TIMEFRAME', '1m');
const DASHBOARD_CHART_LIMIT = Config.number('DASHBOARD_CHART_LIMIT', 120);
const DASHBOARD_AUTH_ENABLED = Config.boolean('DASHBOARD_AUTH_ENABLED', false);
const DASHBOARD_USERNAME = Config.get('DASHBOARD_USERNAME', '');
const DASHBOARD_PASSWORD = Config.get('DASHBOARD_PASSWORD', '');
const DASHBOARD_SESSION_HOURS = Config.number('DASHBOARD_SESSION_HOURS', 12);
...
function validateRuntimeConfiguration() {
  ...
  if (DASHBOARD_ENABLED) {
    requireInteger('DASHBOARD_PORT', DASHBOARD_PORT, 1);
    if (DASHBOARD_PORT > 65535) errors.push('DASHBOARD_PORT must be at most 65535');
    requireInteger('DASHBOARD_REFRESH_SECONDS', DASHBOARD_REFRESH_SECONDS, 2);
    requireInteger('DASHBOARD_CHART_LIMIT', DASHBOARD_CHART_LIMIT, 20);
    requirePositive('DASHBOARD_SESSION_HOURS', DASHBOARD_SESSION_HOURS);
    if (DASHBOARD_AUTH_ENABLED && (!DASHBOARD_USERNAME || !DASHBOARD_PASSWORD)) {
      errors.push('DASHBOARD_USERNAME and DASHBOARD_PASSWORD are required when DASHBOARD_AUTH_ENABLED=true');
    }
    if (DASHBOARD_AUTH_ENABLED && DASHBOARD_PASSWORD.length < 12) {
      errors.push('DASHBOARD_PASSWORD must contain at least 12 characters');
    }
  }
  ...
  if (FIBONACCI_RANGE_ADVISOR_ENABLED) {
    if (!FIBONACCI_RANGE_ADVISOR_TIMEFRAMES.length) {
      errors.push('FIBONACCI_RANGE_ADVISOR_TIMEFRAMES must contain at least one timeframe or "all"');
    }
    if (!FIBONACCI_RANGE_ADVISOR_RATIOS.length ||
        FIBONACCI_RANGE_ADVISOR_RATIOS.some(ratio => !Number.isFinite(ratio) || ratio < 0)) {
      errors.push('FIBONACCI_RANGE_ADVISOR_RATIOS must contain non-negative numeric ratios');
    }
    requireNonNegative(
      'FIBONACCI_RANGE_ADVISOR_CANDLE_CLOSE_BUFFER_MS',
      FIBONACCI_RANGE_ADVISOR_CANDLE_CLOSE_BUFFER_MS
    );
    requirePositive(
      'FIBONACCI_RANGE_ADVISOR_CLUSTER_TOLERANCE_PCT',
    ...
    requireNonNegative(
      'FIBONACCI_RANGE_ADVISOR_REBUILD_COOLDOWN_MS',
      FIBONACCI_RANGE_ADVISOR_REBUILD_COOLDOWN_MS
    );
...
module.exports = {
  ...
  FIBONACCI_RANGE_ADVISOR_ENABLED,
  FIBONACCI_RANGE_ADVISOR_TIMEFRAMES,
  FIBONACCI_RANGE_ADVISOR_RATIOS,
  FIBONACCI_RANGE_ADVISOR_CANDLE_CLOSE_BUFFER_MS,
  FIBONACCI_RANGE_ADVISOR_CLUSTER_TOLERANCE_PCT,
  FIBONACCI_RANGE_ADVISOR_MIN_CLUSTER_SCORE,
