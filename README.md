...
- Futures: `FUTURES_MAX_SYMBOL_EXPOSURE_USDT` dan `FUTURES_MAX_TOTAL_EXPOSURE_USDT` membatasi posisi ditambah BUY tertunda pada simbol yang dikonfigurasi. Nilai posisi memakai harga yang lebih tinggi antara entry dan mark; penurunan harga tidak membuka kuota tambahan. Pemeriksaan memakai data exchange sebelum setiap BUY, termasuk refill. Jika data gagal dibaca, BUY ditunda, SELL tetap eligible. BUY bot yang melampaui kuota dibatalkan dari harga terendah; posisi tidak ditutup paksa. Default mengikuti batas investasi per simbol dan jumlah simbol. Ini batas penambahan exposure, bukan stop-loss atau jaminan nilai posisi tidak berubah akibat pasar.
- Futures: ketika target menguntungkan tidak tersedia di dalam grid, bot mencoba limit SELL di luar grid, dibulatkan ke tick exchange dan tetap terhubung ke sumber BUY. Pembersihan out-of-range tidak membatalkan SELL. Reset range tetap merekonsiliasi dan membuat ulang exit. Order masih tunduk pada batas harga/jumlah dan post-only exchange.
- Telegram dan dashboard futures memisahkan hasil total simbol, hasil Binance setelah fee, hasil posisi yang masih berjalan, serta funding. Profit pasangan grid ditampilkan sebagai statistik terpisah. Target SELL dibandingkan dengan entry rata-rata saat ini; label tersebut belum memperhitungkan fee atau perubahan entry sebelum eksekusi. Notifikasi SELL memakai PnL eksekusi dari Binance, menampilkan fee SELL, dan menjelaskan bahwa sisa LONG dapat tetap terbuka.
- PnL futures pada dashboard/status berasal dari income Binance: `REALIZED_PNL + COMMISSION (bertanda negatif) + rebate + INSURANCE_CLEAR + FUNDING_FEE + unrealized`. Profit pasangan grid disimpan sebagai statistik terpisah dan tidak ditambahkan ke PnL akun. Riwayat mencakup seluruh transaksi simbol (termasuk manual dan SHORT), sejak `createdAt` simbol; bukan seluruh akun atau seluruh masa trading. Unrealized mencakup kedua sisi posisi simbol saat ini. Posisi yang dibuka sebelum periode tersebut dapat memiliki biaya masuk di luar periode. Sinkronisasi setiap 5 menit dengan overlap 7 hari dan deduplikasi ID per jenis income. Data lebih tua dari retensi API, fee non-USDT yang belum dikonversi, atau sinkronisasi kedaluwarsa menghasilkan `N/A`, tidak diasumsikan nol. Fee posisi terbuka tidak dikurangi lagi karena commission sudah tercatat. Deposit/withdraw bukan profit.
