...
function makeConfig(args) {
    return {
        ...
        initialBalance: Number(args.initialBalance || process.env.TUNER_INITIAL_BALANCE || 1000),
        ...
        grid: {
            ...
            atrTpMultiplier: parseListEnv('TUNER_ATR_TP_MULTIPLIER', DEFAULT_GRID.atrTpMultiplier),
            tp1Rr: parseListEnv('TUNER_TP1_RR', DEFAULT_GRID.tp1Rr),
            tp2Rr: parseListEnv('TUNER_TP2_RR', DEFAULT_GRID.tp2Rr),
            sidewaysEmaGap: parseListEnv('TUNER_SIDEWAYS_EMA_GAP', DEFAULT_GRID.sidewaysEmaGap),
            requiredConfirmation: parseListEnv(
                'TUNER_REQUIRED_CONFIRMATION',
                DEFAULT_GRID.requiredConfirmation,
                value => Number.parseInt(value, 10)
            ),
            trailingCallbackMin: parseListEnv('TUNER_TRAILING_CALLBACK_MIN', DEFAULT_GRID.trailingCallbackMin),
            trailingCallbackMax: parseListEnv('TUNER_TRAILING_CALLBACK_MAX', DEFAULT_GRID.trailingCallbackMax)
        }
    };
}
...
function buildSnapshots(ohlcv, lookbackCandles) {
    ...
    for (let i = minCandles; i < ohlcv.length; i++) {
        ...
        snapshots.set(ohlcv[i][0], {
            ...
            volumeChange: prevVolume === 0 ? 0 : ((latestVolume - prevVolume) / prevVolume) * 100,
...
function openSimulatedPosition(signal, candle, snapshot, params, config) {
    ...
    return {
        ...
        sl,
        tp,
        tp1: isLong ? entry + snapshot.atr * params.tp1Rr : entry - snapshot.atr * params.tp1Rr,
        tp2: isLong ? entry + snapshot.atr * params.tp2Rr : entry - snapshot.atr * params.tp2Rr,
        qty,
        remainingQty: qty,
        tp1Qty,
        tp2Qty,
...
function settleQty(position, qty, price, feeRate) {
    ...
    const gross = position.signal === 'LONG'
...
function updatePosition(position, candle, feeRate) {
    ...
    if (isLong) {
        if (low <= position.sl) {
            position.realizedPnl += settleQty(position, position.remainingQty, position.sl, feeRate);
            return closeTrade(position, candle[0], position.sl, 'SL');
        }

        if (!position.tp1Filled && high >= position.tp1) {
            position.realizedPnl += settleQty(position, position.tp1Qty, position.tp1, feeRate);
            position.tp1Filled = true;
        }

        if (!position.tp2Filled && high >= position.tp2) {
            position.realizedPnl += settleQty(position, position.tp2Qty, position.tp2, feeRate);
        ...
        if (position.tp1Filled && position.remainingQty > 0 && low <= trailingStop) {
            position.realizedPnl += settleQty(position, position.remainingQty, trailingStop, feeRate);
            return closeTrade(position, candle[0], trailingStop, 'TRAILING');
        }

        if (high >= position.tp && position.remainingQty > 0) {
            position.realizedPnl += settleQty(position, position.remainingQty, position.tp, feeRate);
        ...
        if (position.tp1Filled && position.remainingQty > 0 && high >= trailingStop) {
            position.realizedPnl += settleQty(position, position.remainingQty, trailingStop, feeRate);
            return closeTrade(position, candle[0], trailingStop, 'TRAILING');
        }
...
function backtest(baseOhlcv, baseSnapshots, htfSnapshots, htfTimestamps, params, config) {
    ...
    for (let i = 0; i < baseOhlcv.length - 1; i++) {
        ...
        const htfSnapshot = getLatestSnapshotBefore(htfSnapshots, htfTimestamps, candle[0]);
        ...
        position = openSimulatedPosition(signal, nextCandle, snapshot, params, config);
    ...
    if (position) {
        ...
        position.realizedPnl += settleQty(position, position.remainingQty, lastCandle[4], config.feeRate);
...
function makeParamGrid(grid) {
    ...
    for (const trailingCallbackMax of grid.trailingCallbackMax) {
        if (tp2Rr <= tp1Rr || trailingCallbackMax < trailingCallbackMin) {
            continue;
        }
...
async function main() {
    ...
    console.log(`Fetching ${config.symbol} ${config.timeframe}/${config.htfTimeframe} for ${config.days} days...`);
    ...
    if (baseOhlcv.length < config.lookbackCandles + 20) {
        throw new Error(`Not enough ${config.timeframe} candles: ${baseOhlcv.length}`);
    }

    if (htfOhlcv.length < 80) {
        throw new Error(`Not enough ${config.htfTimeframe} candles: ${htfOhlcv.length}`);
    }
    ...
    const htfSnapshots = buildSnapshots(htfOhlcv, Math.min(config.lookbackCandles, 120));
    ...
    const results = paramGrid
        .map(params => backtest(baseOhlcv, baseSnapshots, htfSnapshots, htfTimestamps, params, config))
        .filter(result => result.trades >= config.minTrades)
        .sort((a, b) => b.score - a.score)
        .slice(0, config.top);

    if (!results.length) {
        throw new Error(`No config reached TUNER_MIN_TRADES=${config.minTrades}. Try more days or lower --min-trades.`);
    }
